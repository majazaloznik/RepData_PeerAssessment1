...
The mean total number of steps per day is `r round(mean.perday,2)` and the median `r median.perday` stes per day. 
...
The largest average number of steps across all days is in interval `r max.interval`. 
...
The mean total number of steps per day is `r round(mean.perday.new,2)` and the median `r round(median.perday.new,2)` stes per day. The mean is therefore slightly higher than before the imputation, while the median has stayed the same. 
